       ...
       <div id="h">반려견 관리&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>
        ...
        <table>
                <tr>
                        ...
                        <% for(Pet pet : p){%>
                        <form id="joinForm" name="joinForm" action="<%= request.getContextPath() %>/myPetInfo2" method="post">
                        <input type="hidden" name="dogNum" value="<%=pet.getDogNum() %>"></input>
                        <button><%=pet.getDogName() %></button></form><%} %></td>
...
<div id="acc">
    <%if(selectPet==null) {%>
		    <button type="button" onclick="InsertPet();">추가하기</button> 
	    <form action="<%= request.getContextPath() %>/myPetUpdateServlet" method="post">
		    <input type="hidden" name="dogNum" value="<%=p.get(0).getDogNum() %>">
		    <button onclick="UpdatePet();">수정하기</button> 
		</form>
		<form action="<%= request.getContextPath() %>/myPetDeleteServlet" method="post" onsubmit="return deleteConfirm();">
			<input type="hidden" name="dogNum" value="<%=p.get(0).getDogNum() %>">
		    <button>삭제하기</button>
		</form>	
</div>
